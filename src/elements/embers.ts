...
export class Embers extends View {
	...
	createNativeView(): any {
		...
		// A radial GradientDrawable paints nothing until it has a radius; size it
		// from the native layout pass, which is the only one a plain View gets.
		glow.setGradientCenter(0.5, 1);
		const view = new android.view.View(this._context);
		view.setBackground(glow);
		view.addOnLayoutChangeListener(
			new android.view.View.OnLayoutChangeListener({
				onLayoutChange: (changed: android.view.View) => {
					glow.setGradientRadius(Math.max(1, changed.getHeight() * 0.9));
				},
			}),
		);
		return view;
	...
	private applyHeat(value: number = this.heat): void {
		if (!isIOS) {
			(this.nativeViewProtected as unknown as android.view.View).setAlpha(Math.min(1, Math.max(0, value * 0.45)));
			return;
		}
		const emitter = this.emitter;
		if (emitter === null) return;
